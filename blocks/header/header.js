...
function openOnHover(p) {
  if (isDesktop.matches) {
    p.setAttribute('aria-expanded', 'true');
  }
}

function closeOnHover(p) {
  if (isDesktop.matches) {
...
function toggleMenu(nav, navSections, forceExpanded) {
  ...
  // enable menu close on escape keypress
...
export default async function decorate(block) {
  ...
  if (navSections) {
    navSections.querySelectorAll(':scope .default-content-wrapper > ul > li').forEach((navSection) => {
      ...
      navSection.addEventListener('mouseenter', () => {
        openOnHover(navSection);
      });
      navSection.addEventListener('mouseleave', () => {
        closeOnHover(navSection);
      });
    });
  }
  ...
  // prevent focus highlight during mouse click
  hamburger.addEventListener('mousedown', (_e) => {
    _e.preventDefault();
  });
